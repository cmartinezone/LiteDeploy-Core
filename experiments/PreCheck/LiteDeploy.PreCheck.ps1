                ...
                <Border Grid.Row="2" Background="$footerBg" Padding="20,12" BorderBrush="$borderColor" BorderThickness="0,1,0,0">
                    <Grid>
                        ...
                        <StackPanel Grid.Column="1" Orientation="Horizontal">
                            <Button Name="BtnDiagnostics" Content="Diagnostics" Style="{StaticResource SecondaryButtonStyle}" Margin="0,0,10,0"/>
...
function Test-NetworkHardware([string]$Mode) {
    try {
        ...
        Add-Result "Network Adapter: Not connected" $status
        return ($Mode -eq "Media")
    }
    catch {
        $status = if ($Mode -eq "Media") { "WARN" } else { "FAIL" }
        Add-Result "Network Adapter: Unable to detect" $status
        return ($Mode -eq "Media")
    }
}
...
function Test-NetworkIPAddress([int]$TimeoutSeconds, [int]$PollIntervalMs, [string]$Mode) {
    try {
        ...
        $status = if ($Mode -eq "Media") { "WARN" } else { "FAIL" }
        Add-Result "IPv4 Address: Unable to detect" $status
        return ($Mode -eq "Media")
    }
}
...
function Test-InternalStorage([int]$MinimumGB) {
    ...
    if (-not $items) { Add-Result "Hard Drive Available: Not detected" "FAIL"; return $false }
    $valid = 0
    foreach ($item in $items) {
        ...
        Add-Result "Hard Drive Available: Disk $($item.Number) - $($item.Model)$sizeText" $status
    }
    $valid -gt 0
}
...
function Test-SystemEnvironment {
    try {
        ...
        if ($firmware -eq "UEFI") {
            ...
            Add-Result "Secure Boot: $secureBoot" $(if ($secureBoot -eq "Enabled") { "OK" } else { "WARN" })
        }
        else { Add-Result "BIOS Mode: $firmware (Legacy BIOS)" "WARN" }
        $true
    }
...
function Invoke-PreCheck {
    ...
    Set-PreCheckProgress 80 "Validating internal storage and system memory..."
    if (-not (Test-InternalStorage $MinDiskSizeGB)) { $global:PreCheckPassed = $false }
    Test-SystemMemory $MinMemoryGB | Out-Null
    Set-PreCheckProgress 95 "Analyzing firmware and Secure Boot..."
    ...
    if ($global:PreCheckPassed) {
        ...
        if ($HaltOnFailure) {
            $btnContinue.IsEnabled = $false; $btnContinue.Content = "Blocked"
        }
        else {
            $btnContinue.IsEnabled = $true; $btnContinue.Content = "Continue"
        }
...
if ($null -ne $btnDiagnostics) { $btnDiagnostics.Add_Click({ Start-Process "$env:SystemRoot\System32\cmd.exe" }) }
