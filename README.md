...
| — | [ImportOSMedia](components/Manager/ImportOSMedia) | Imports OS media and publishes the catalog SelectWorkflow and the engine consume | Exists |
...
`ImportOSMedia` (in `components/Manager/ImportOSMedia`) sits after Config. It publishes the OS catalog that SelectWorkflow and the engine will consume.
...
```text
LiteDeploy Core/
  components/                 Numbered in working order and management tools
    01-Config/
    02-DeploymentShareACL/
    03-LogWriter/
    04-HostShell/
    05-BootInitializer/
    06-PreCheck/
